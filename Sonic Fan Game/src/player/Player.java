...
import static functionholders.CollisionFunctions.clip;
import static functionholders.GeometryFunctions.rotateRectangle;
...
public class Player {
    ...
    public Player(double x, double y) {
        ...
        skidFastAnim = new Animation(Loader.skidFastAnim, new int[]{2, 2, 2, 2, 2, 2}, 3);
    ...
    public void update(float dt, Shape[] layer0, Shape[] layer1, Shape[] layer2, Shape[] layer1Triggers, Shape[] layer2Triggers, Shape[] platforms, Ring[] rings, Spring[] springs, Badnik[] badniks, Item[] items, Ramp[] ramps, Rotor[] rotors, SpringPole[] springPoles, Helix[] helixes, DashPad[] dashPads, Rail[] rails, BlueSpring[] blueSprings) {
        ...
        if (ground || anim != JUMP_ANIM) {
            doubleSpinReady = false;
        }
        if (ground || state != STATE_SPINNING) {
        ...
        if ((stopCam || state == STATE_SPRING_POLING) && state != STATE_SMASHING_START && state != STATE_SMASHING && state != STATE_SMASHING_END) {
            vel = new Vector();
        }
        if (helixing) {
            vel.y = 0;
        ...
        if (platforms != null) {
            platMasks = checkPlatforms(platforms);
        }
        ...
        Shape[] shapes = null;
        if (layer == 1) {
            shapes = combine(layer0, layer1);
        }
        if (layer == 2) {
            shapes = combine(layer0, layer2);
        }
        if (platMasks != null) {
        ...
        if (rails != null) {
            for (int i = 0; i < rails.length; i++) {
                ...
                for (int s = 0; s < railShapes.length; s++) {
                    shapes = append(shapes, railShapes[s]);
                }
            }
        }
    ...
    private void afterImages(float dt) {
        ...
        if (boostMode) {
            if (anim == SWING_ANIM) {
                ...
                afters = append(afters, new AfterImage(getCurrentAnim().getCurrentFrame(), pos.x - w / 2, pos.y - h / 2 - 32 + 2, pos.x, pos.y, t, -facing * 2, 2, 2, 5));
            }
        } else {
            afters = null;
        }
        ...
        if (afters != null) {
            ...
            if (removals != null) {
                for (int i = 0; i < removals.length; i++) {
                    afters = removeIndex(afters, removals[i]);
                }
            }
        }
    }
    ...
    private void checkLayer(Shape[] layer1Triggers, Shape[] layer2Triggers) {
        ...
        if (layer2Triggers != null) {
            for (int i = 0; i < layer2Triggers.length; i++) {
                if (checkCollision(mask, layer2Triggers[i])) {
                    layer = 2;
                }
            }
        }
        if (layer1Triggers != null) {
            for (int i = 0; i < layer1Triggers.length; i++) {
                if (checkCollision(mask, layer1Triggers[i])) {
                    layer = 1;
                }
            }
        }
    ...
    private void collide(Shape[] shapes) {
        ...
        if (dir.getLength() != 0) {
            if (state != STATE_SMASHING) {
                ...
                if (getDistanceBetweenAngles(initial, normal + PI / 2) < getDistanceBetweenAngles(initial, normal - PI / 2)) {
                    finish -= dif0 * 2;
                } else if (getDistanceBetweenAngles(initial, normal + PI / 2) > getDistanceBetweenAngles(initial, normal - PI / 2)) {
                    finish += dif0 * 2;
                }
    ...
    private boolean[] checkPlatforms(Shape[] shapes) {
        ...
        for (int i = 0; i < shapes.length; i++) {
            ...
            for (int p = 0; p < shapes[i].points.length; p++) {
                if (pos.y + MASK_RADIUS * SCALE > shapes[i].points[p].y) {
                    out[i] = false;
                }
            }
        }
    ...
    private void checkLedge(Shape[] shapes) {
        ...
        for (int i = 0; i < shapes.length; i++) {
            if (checkCollision(groundMask0, shapes[i])) {
                ground0 = true;
            }
        }
        for (int i = 0; i < shapes.length; i++) {
            if (checkCollision(groundMask1, shapes[i])) {
                ground1 = true;
            }
        }
    ...
    private void checkGround(Shape[] shapes) {
        ...
        if (state != STATE_SMASHING) {
            for (int i = 0; i < shapes.length; i++) {
                if (checkCollision(groundMask, shapes[i])) {
                    ground = true;
                }
            }
    ...
    private void getGroundAxis(Shape[] shapes) {
        if (ground && !ledge) {
            ...
            if (abs(groundSpeed) < STICK_MIN_SPEED * SCALE) {
                groundAxis = new Vector(0, 1);
            }
            ...
            if (groundAxis.getLength() != 0) {
                groundAxis = groundAxis.scale(-1).normalize();
            } else {
                groundAxis = new Vector(0, 1);
            }
    ...
    private Animation getCurrentAnim() {
        if (anim == RUN_ANIM) {
            if (abs(groundSpeed) >= FASTEST_MIN_SPEED * SCALE) {
                return (runFastestAnim);
            } else if (abs(groundSpeed) >= FAST_MIN_SPEED * SCALE) {
                return (runFastAnim);
            } else if (abs(groundSpeed) >= NORMAL_MIN_SPEED * SCALE) {
                return (runNormalAnim);
            } else if (abs(groundSpeed) >= SLOW_MIN_SPEED * SCALE) {
                return (runSlowAnim);
            } else {
                return (runSlowestAnim);
            }
        }
        ...
        if (anim == SPINDASH_ANIM) {
            return (spindashAnim);
        }
        if (anim == SPINDASH_CHARGE_ANIM) {
            return (spindashChargeAnim);
        }
        if (anim == JUMP_ANIM) {
            return (jumpAnim);
        }
        ...
        if (anim == TRICK_RIGHT_ANIM) {
            return (trickRightAnim);
        }
        if (anim == TRICK_UP_0_ANIM) {
            return (trickUp0Anim);
        }
        ...
        if (state != STATE_STARTING) {
            if (state == STATE_SWINGING) {
                ...
                if (chargeDustTimer == 0) {
                    if (dustAnim == REGULAR_DUST_ANIM) {
                        spindashDustAnim.update(1);
                    } else {
                        dustAnim = REGULAR_DUST_ANIM;
                        spindashDustAnim.reset();
                    }
                } else {
                    if (dustAnim == CHARGE_DUST_ANIM) {
                        spindashChargeDustAnim.update(1);
                    } else {
                ...
                if (doubleShieldDrawn) {
                    if (!doubleShieldSpriteActive) {
                        ...
                        if (doubleShieldAnim.finished) {
                            doubleShieldDrawn = false;
                        }
                    }
                }
                if (!doubleShieldDrawn) {
                    doubleShieldSpriteActive = false;
                }
                ...
                if (ground) {
                    if (groundSpeed == 0 && state != STATE_TURNING_SLOW && state != STATE_TURNING_FAST && state != STATE_SKIDDING_SLOW && state != STATE_SKIDDING_FAST) {
                        ...
                        if (state == STATE_SKIDDING_SLOW || state == STATE_SKIDDING_FAST || state == STATE_TURNING_FAST) {
                            if (state == STATE_SKIDDING_SLOW || state == STATE_SKIDDING_FAST) {
                                ...
                                if (state == STATE_SKIDDING_FAST) {
                                    if (anim == SKID_FAST_ANIM) {
                                        skidFastAnim.update(1);
                                    } else {
                                        anim = SKID_FAST_ANIM;
                                        skidFastAnim.reset();
                                        ps.playSound(SOUND_SKID);
                        ...
                        if (state == STATE_BOUNCING && bounceType == 0) {
                            if (vel.y < 0) {
                                ...
                                if (anim != BOUNCING_DOWN_ANIM) {
                                    ...
                                    if (bounceDownAnim.finished) {
                                        state = STATE_DEFAULT;
                                    }
                                }
                            }
                        } else if (state == STATE_BOUNCING && bounceType == 1) {
                            if (anim == SPIN_ANIM) {
                                spinAnim.update(1);
                            }
                        }
    ...
    public void draw(float dt, Renderer r) {
        ...
        if (DRAW_SPRITES) {
            ...
            if (anim == RUN_ANIM) {
                if (abs(groundSpeed) >= FASTEST_MIN_SPEED * SCALE) {
                    runFastestAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
                } else if (abs(groundSpeed) >= FAST_MIN_SPEED * SCALE) {
                    runFastAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
                } else if (abs(groundSpeed) >= NORMAL_MIN_SPEED * SCALE) {
                    runNormalAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
                } else if (abs(groundSpeed) >= SLOW_MIN_SPEED * SCALE) {
                    runSlowAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
                } else {
                    runSlowestAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
                }
            }

            if (anim == START_ANIM) {
                startAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == IDLE_ANIM) {
                idleAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == BOUNCING_UP_ANIM) {
                bounceUpAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == BOUNCING_DOWN_ANIM) {
                bounceDownAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == FALL_ANIM) {
                fallAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SKID_ANIM) {
                skidAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SKID_FAST_ANIM) {
                skidFastAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SKIRT_ANIM) {
                skirtAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == TURN_ANIM) {
                turnAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == CROUCH_ANIM_0) {
                crouchAnim0.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == CROUCH_ANIM_1) {
                crouchAnim1.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == JUMP_ANIM) {
                jumpAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3 + 8) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == LAND_ANIM) {
                landAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3 + 8) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == TRICK_RIGHT_ANIM) {
                trickRightAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == TRICK_UP_0_ANIM) {
                trickUp0Anim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == TRICK_UP_1_ANIM) {
                trickUp1Anim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == RAMP_ANIM) {
                rampAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == DASH_ANIM) {
                dashAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3 + 8) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == DOUBLE_SPIN_ANIM) {
                doubleSpinAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3 + 8) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == TRICK_BACK_ANIM) {
                backflipAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SLIDE_ANIM) {
                slideAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SLAM_START_ANIM) {
                smashStartAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SLAM_END_ANIM) {
                smashEndAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == GRIND_ANIM) {
            ...
            if (anim == SPIN_ANIM) {
                spinAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 - 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, 0, -facing * Loader.scale, Loader.scale, r);
            }

            if (anim == SPINDASH_ANIM) {
                spindashAnim.draw((pos.x - w / 2 + s) / 2 * Loader.scale, (pos.y - h / 2 - 32 - 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (anim == SPINDASH_CHARGE_ANIM) {
                spindashChargeAnim.draw((pos.x - w / 2 + s) / 2 * Loader.scale, (pos.y - h / 2 - 32 - 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (dustAnim == REGULAR_DUST_ANIM && ground) {
                spindashDustAnim.draw((pos.x - w / 2 + s) / 2 * Loader.scale, (pos.y - h / 2 - 32 - 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
            if (dustAnim == CHARGE_DUST_ANIM && ground) {
                spindashChargeDustAnim.draw((pos.x - w / 2 + s) / 2 * Loader.scale, (pos.y - h / 2 - 32 - 3) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }

            if (doubleShieldSpriteActive) {
                doubleShieldAnim.draw((pos.x - w / 2) / 2 * Loader.scale, (pos.y - h / 2 - 32 + 3 + 8) / 2 * Loader.scale + 0, pos.x / 2 * Loader.scale, pos.y / 2 * Loader.scale, t, -facing * Loader.scale, Loader.scale, r);
            }
        }
    }
