...
public class PlayerActions {
    public static void movement(Player p) {
        if (!p.ground && (p.state == STATE_SKIDDING_SLOW || p.state == STATE_TURNING_SLOW || p.state == STATE_TURNING_FAST || p.state == STATE_SLIDING)) {
        ...
        if (p.state != STATE_CROUCHING_DOWN && p.state != STATE_CROUCHING && p.state != STATE_CROUCHING_UP && p.state != STATE_SPINDASHING) {
            if ((p.state != STATE_SPINNING || !p.ground) && p.state != STATE_RAMP_DASHING && p.state != STATE_DASHING && p.state != STATE_SLIDING) {
                defaultMovement(p);
    ...
    private static void defaultMovement(Player p) {
        ...
        if (p.leftArrow && !p.rightArrow) {
            if (p.groundSpeed <= 0 || !p.ground) {
                if (!p.ground && p.state != STATE_TRICKING_UP && p.state != STATE_TRICKING_BACKWARD && p.state != STATE_TRICKING_FORWARD) {
                    p.facing = -1;
                }
                // turn around
                if (p.state == STATE_SKIDDING_SLOW || p.state == STATE_SKIDDING_FAST) {
                    if (p.facing == 1) {
                        p.state = STATE_TURNING_FAST;
                    } else {
                        p.state = STATE_DEFAULT;
                    }
                }
                ...
                if (p.groundSpeed > -GROUND_ACCEL_LIMIT * capScale * SCALE || p.shiftKey) {
                    if (p.ground || p.shiftKey) {
                        p.groundSpeed -= moveSpeed * accelScale;
                    }
                    if (!p.ground) {
                        p.groundSpeed -= AIR_ACCEL * accelScale;
                    }
                    if (p.groundSpeed < -GROUND_ACCEL_LIMIT * capScale * SCALE && !p.shiftKey) {
                        p.groundSpeed = -GROUND_ACCEL_LIMIT * capScale * SCALE;
                    }
                }
            } else {
                p.groundSpeed -= SKID_ACCEL * SCALE;
                if (p.groundSpeed >= SKID_MIN_SPEED) {
                    if (!(abs(p.groundSpeed) >= FASTEST_MIN_SPEED * SCALE)) { // TODO: give fastest movement a proper flag
                        if (p.state != STATE_SKIDDING_FAST)
                            p.state = STATE_SKIDDING_SLOW;
                    } else {
                        p.state = STATE_SKIDDING_FAST;
                    }
                }
            }
        }
    ...
    private static void slidingMovement(Player p) {
        if (p.leftArrow && !p.rightArrow && p.ground || !p.leftArrow && p.rightArrow && p.ground) {
            if (p.groundSpeed > 0) {
                p.groundSpeed -= SLIDE_DECEL * SCALE;
            } else if (p.groundSpeed < 0) {
                p.groundSpeed += SLIDE_DECEL * SCALE;
            }
    ...
    public static void drag(Player p) {
        if (!p.leftArrow && !p.rightArrow && p.ground || p.leftArrow && p.rightArrow && p.ground) {
            ...
            if (p.state != STATE_SPINNING && p.state != STATE_SPINDASHING && p.state != STATE_SLIDING) { // regular drag
                if (p.groundSpeed > 0) {
                    p.groundSpeed -= DRAG_DECEL * SCALE;
                } else if (p.groundSpeed < 0) {
                    p.groundSpeed += DRAG_DECEL * SCALE;
                }

                if (p.groundSpeed >= -DRAG_DECEL * SCALE && p.groundSpeed <= DRAG_DECEL * SCALE) {
                    p.groundSpeed = 0;
                }
            } else if (p.state == STATE_SPINNING) { // spinning drag
                if (p.groundSpeed > 0) {
                    p.groundSpeed -= SPIN_DECEL * SCALE;
                } else if (p.groundSpeed < 0) {
                    p.groundSpeed += SPIN_DECEL * SCALE;
                }
    ...
    public static void jump(Player p) {
        ...
        if (p.jumpReady && p.spaceBar && p.state != STATE_CROUCHING_UP && p.state != STATE_CROUCHING && p.state != STATE_CROUCHING_DOWN && p.ground && p.state != STATE_SPINDASHING) {
            ...
            if (p.jumpingUp) {
                ...
                if (p.jumpSlowed >= 0 && (p.anim == JUMP_ANIM || p.anim == LAND_ANIM)) {
                    p.jumpingUp = false;
                }
            } else {
                p.jumpSlowing = false;
    ...
    public static void trick(Player p) {
        ...
        if ((p.state == STATE_BOUNCING || p.state == STATE_RAMP_DASHING) && !p.spaceBar) {
            p.trickReady = true;
        }
    }
    ...
    public static void dash(Player p) {
        if (p.controlKey && p.dashReady && p.state == STATE_JUMPING && (p.anim == JUMP_ANIM || p.anim == LAND_ANIM)) {
            ...
            if (p.groundSpeed > GROUND_ACCEL_LIMIT * SCALE && !p.boostMode) {
                p.groundSpeed = GROUND_ACCEL_LIMIT * SCALE;
            }
            if (p.groundSpeed > GROUND_ACCEL_LIMIT * BOOST_LIMIT_SCALE * SCALE && p.boostMode) {
                p.groundSpeed = GROUND_ACCEL_LIMIT * BOOST_LIMIT_SCALE * SCALE;
            }
            p.vel.y = 0;
    ...
    public static void gravity(Player p) {
        ...
        if (!p.ground) {
            ...
            if (p.jumpingUp) {
                p.jumpSlowed += GRAVITY * SCALE;
            }

            if (p.vel.x < -GROUND_ACCEL_LIMIT * capScale * SCALE && !p.shiftKey && p.state != STATE_SPINNING) {
                p.vel.x = -GROUND_ACCEL_LIMIT * capScale * SCALE;
            }
            if (p.vel.x > GROUND_ACCEL_LIMIT * capScale * SCALE && !p.shiftKey && p.state != STATE_SPINNING) {
                p.vel.x = GROUND_ACCEL_LIMIT * capScale * SCALE;
            }
            ...
            if (tempGrav.getLength() >= MIN_POTENTIAL_GRAVITY) {
                p.groundSpeed += p.getRotatedVectorComponents(tempGrav, p.groundAxis).x;
            }

            if (p.groundSpeed < -GROUND_ACCEL_LIMIT * capScale * SCALE && !p.shiftKey && p.state != STATE_SPINNING) {
                p.groundSpeed = -GROUND_ACCEL_LIMIT * capScale * SCALE;
            }
            if (p.groundSpeed > GROUND_ACCEL_LIMIT * capScale * SCALE && !p.shiftKey && p.state != STATE_SPINNING) {
                p.groundSpeed = GROUND_ACCEL_LIMIT * capScale * SCALE;
    ...
    public static void boost(Player p) {
        if (p.ground) {
            if (!p.boostMode) {
                ...
                if (abs(p.groundSpeed) < BOOST_STOP_SPEED) {
                    p.boostMode = false;
                }
            }
        }
    ...
    public static void doubleSpin(Player p) {
        if (p.state != STATE_BOUNCING && !p.ground && p.anim == JUMP_ANIM && !p.doubleSpinning) {
            ...
            if (!p.spaceBar) {
                p.doubleSpinReady = true;
            } else {
                p.doubleSpinReady = false;
            }
        } else {
            p.doubleSpinReady = false;
        }

        if (p.ground || p.state != STATE_SPINNING) {
            p.doubleSpinning = false;
        }
        if (!p.doubleSpinning) {
